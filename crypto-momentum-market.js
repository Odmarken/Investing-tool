...
export async function fetchDerivatives(grab,symbol,position,now){
  ...
  const get=async(path,params)=>{
    const j=await grab(api+path+'?'+new URLSearchParams({category:'linear',symbol:contractSymbol,...params}),{json:true,timeout:8000});
  ...
  const info=await get('instruments-info',{}),instrument=info.result.list.find(x=>x.symbol===contractSymbol);
  ...
  if(allFunds.at(-1).t<nextFunding-interval||allFunds.some((f,i)=>i&&f.t>position.fundingThrough&&f.t-allFunds[i-1].t>interval))
