...
const RULES = [
  ...
  {k:/(?=.*\b(bitcoin|btc|ethereum|eth|solana|crypto|altcoin|xrp|doge|exchange|coinbase|binance|bybit|kraken|stablecoin|defi|token)\b).*\b(lawsuit|sues?|sued|charged|charges|indict\w*|subpoena\w*|crackdown|bans?|banned|fine[sd]?|halts?|rejects?|rejected|delays?|delayed|seiz\w*|arrest\w*)\b/i, nq:0, gc:0, kr:-2, c:'krypto', sv:'Regulatoriskt tryck → osäkerhet om tillgång till marknaden. Negativt, mest för de mindre coinsen.'},
  {k:/\b(sec|cftc|regulators?|court|judge|senate|house|congress)\b.*\b(approv\w*|greenlight\w*|clears?|cleared|allows?|allowed|permits?|window|guidance|clarity|passes|passed|dismiss\w*|drops? (?:the )?(?:case|lawsuit|charges))\b/i, nq:0, gc:0, kr:2, c:'krypto', sv:'Myndighet eller domstol som öppnar → lägre regulatorisk risk. Positivt, mest för de stora.'},
  {k:/\b(regulat\w* clarity|licen[cs]e[sd]?|licensed|greenlight\w*|(?:bill|act|law) (?:passes|passed|signed|approved)|(?:senate|house|congress|parliament) (?:passes|passed|approves|approved)|mica)\b/i, nq:0, gc:0, kr:2, c:'krypto', sv:'Regulatorisk klarhet eller licens → lägre risk för sektorn. Positivt över tid, ofta måttligt i 5m.'},
  ...
  {k:/(?=.*\b(bitcoin|btc|ethereum|eth|solana|crypto|altcoin|xrp|doge)\b).*\b(bargain|undervalued|buy the dip|accumulation|bullish|bull run|bull market|adoption)\b/i, nq:0, gc:0, kr:1, c:'krypto', sv:'Positiv ton — köpläge, bullish. Sentimentsignal, inte flöde. Svag.'},
  ...
  {k:/(?=.*\b(bitcoin|btc|ethereum|eth|solana|crypto|altcoin|xrp|doge)\b).*\b(strategic (?:bitcoin |crypto )?reserve|(?:bitcoin|btc|crypto|ether|eth) treasury|treasury (?:compan(?:y|ies)|strategy|purchase[sd]?|reserve)|(?:adds?|acquires?|bought|purchases?) (?:[\d,.]+[kmb]?\s*)?(?:more )?(?:bitcoin|btc|ether|eth)\b|buys? (?:[\d,.]+[kmb]?\s*)?(?:more )?(?:bitcoin|btc|ether|eth)\b)\b/i, nq:0, gc:0, kr:2, c:'krypto', sv:'Stat, bolag eller fond som köper eller reserverar → strukturell efterfrågan. Tydligt positiv.'},
